...
#include <fstream>
#include <ip.hpp>
#include <fort_cover_ip.hpp>
#define STOPPING_TIME 7200
using namespace std;
using namespace ip;
using namespace fcip;
/* cut_callback */
int __stdcall fcip::cut_callback(GRBmodel *model, void *cbdata, int where, void *usrdata){
...
/* zero_forcing */
int fcip::zero_forcing(ip::graph &our_graph)
{
	GRBenv *env = NULL;
...
//*/
 ...
 // error = GRBsetintparam(GRBgetenv(zero_forcing), "RINS", 5);
  ...
  error = GRBsetcallbackfunc(zero_forcing, fcip::cut_callback, (void *) &mydata);
  if(error){
